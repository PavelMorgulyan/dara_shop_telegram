...
want_another_ai_img = {
    "want_another_ai_img": 'Хочу еще раз попробовать создать эскиз'
}
...
kb_phone_number = ReplyKeyboardMarkup(resize_keyboard=True).add(
    KeyboardButton(phone_number['client_send_contact'], request_contact=True)
).add(
    KeyboardButton(phone_number['client_dont_send_contact'])
)
...
kb_choice_get_photo_for_place_tattoo = create_kb(list(choice_get_photo_for_place_tattoo.values()) \
    + back_lst + cancel_lst)
kb_want_another_ai_img = create_kb(list(want_another_ai_img.values()) + back_lst + cancel_lst)
kb_correct_photo_from_ai_or_get_another = create_kb(list(correct_photo_from_ai_or_get_another.values())\
    + back_lst + cancel_lst)
kb_no_tattoo_note_from_client = create_kb(no_tattoo_note_from_client + back_lst + cancel_lst)
...
kb_yes_no = ReplyKeyboardMarkup(resize_keyboard=True).row(yes, no).add(back_btn).add(cancel_btn)
kb_have_yes_no = ReplyKeyboardMarkup(resize_keyboard=True)
kb_pay_now_later = ReplyKeyboardMarkup(resize_keyboard=True).row(now, later).add(back_btn).add(cancel_btn)
...
kb_choice_place_tattoo = create_kb(list(choice_place_tattoo.values()) + \
    back_lst + cancel_lst)
kb_client_choice_main_or_temporary_tattoo = create_kb(list(choice_main_or_temporary_tattoo.values()) \
    + cancel_lst)
...
'''
kb_number_tattoo_details = create_kb(list(number_tattoo_details.values()) + back_lst + cancel_lst)
...
kb_start_dialog_sketch_order = create_kb( list(start_dialog_sketch_order.values()) + cancel_lst)

kb_client_size_tattoo = create_kb(list(size_dict.values()) + back_lst + cancel_lst)

kb_another_size = create_other_size_btn()

kb_client_choice_send_more_photo_to_skatch_order = \
    create_kb( list(client_choice_send_more_photo_to_skatch_order.values()) + back_lst +cancel_lst)
