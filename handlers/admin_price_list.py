...
async def get_price_list_commands(message: types.Message):
    if (
        message.text in ["Прайс-лист", "/прайслист"]
...
async def command_create_new_price_list_to_tattoo_order(message: types.Message):
    if (
        message.text in ["создать новый прайс-лист", "/создать_новый_прайслист"]
        and str(message.from_user.username) in ADMIN_NAMES
    ):
...
async def get_to_view_price_list(message: types.Message):
    if (
        message.text in ["посмотреть прайс-лист", "/посмотреть_прайслист"]
        and str(message.from_user.username) in ADMIN_NAMES
    ):
    ...
    if message.text in [
        ...
        with Session(engine) as session:
            prices = session.scalars(
                ...
                .order_by(OrderPriceList.max_size)
            ).all()
        ...
        with Session(engine) as session:
            prices = session.scalars(
                select(OrderPriceList).where(OrderPriceList.type == message.text)
            ).all()
            if prices == []:
...
async def delete_price_list(message: types.Message):
    if (
        message.text in ["удалить прайс-лист", "/удалить_прайслист"]
        and str(message.from_user.username) in ADMIN_NAMES
    ):
        await bot.send_message(
            message.from_id,
...
async def get_name_for_deleting_price_list(message: types.Message, state: FSMContext):
    ...
    if message.text in price_list:
        with Session(engine) as session:
            price = session.scalars(
                select(OrderPriceList)
                .where(OrderPriceList.min_size == message.text.split()[2])
                .where(OrderPriceList.max_size == message.text.split()[6])
            ).one()
...
async def change_price_list(message: types.Message):
    if (
        message.text in ["изменить прайс-лист", "/изменить_прайслист"]
        and str(message.from_user.username) in ADMIN_NAMES
    ):
...
async def get_price_list_name_to_change(message: types.Message, state: FSMContext):
    ...
    if message.text in [
        ...
        with Session(engine) as session:
            price_lst = session.scalars(
                select(OrderPriceList).where(OrderPriceList.type == price_lst_type)
            ).one()
...
async def get_new_status_price_list(message: types.Message, state: FSMContext):
    if message.text in ["Минимальный размер", "Макcимальный размер", "Цена"]:
        ...
        await message.reply(
            "⭕️ Пожалуйста, выбери из представленных вариантов",
            reply_markup=kb_admin.kb_change_price_list,
        )
...
async def update_new_status_price_list(message: types.Message, state: FSMContext):
    if message.text in kb_admin.another_price_lst:  # 'Другая цена'
        ...
        await state.finish()
        await message.reply(
            MSG_CANCEL_ACTION + MSG_DO_CLIENT_WANT_TO_DO_MORE,
            reply_markup=kb_admin.kb_price_list_commands,
        )
...
def register_handlers_admin_price_list(dp: Dispatcher):
    ...
    dp.register_message_handler(
        command_create_new_price_list_to_tattoo_order,
        Text(equals="создать новый прайс-лист", ignore_case=True),
        state=None,
    )
    dp.register_message_handler(
        command_create_new_price_list_to_tattoo_order,
    ...
    dp.register_message_handler(
        get_price_list_name, state=FSM_Admin_create_price_list.get_price_list_name
    )
    dp.register_message_handler(
        get_min_size_to_new_price_list,
        state=FSM_Admin_create_price_list.get_min_size_to_new_price_list,
    )
    dp.register_message_handler(
        get_max_size_to_new_price_list,
    ...
    dp.register_message_handler(
        ...
        Text(equals="посмотреть прайс-лист", ignore_case=True),
        state=None,
    )
    dp.register_message_handler(
    ...
    dp.register_message_handler(
        delete_price_list,
        Text(equals="удалить прайс-лист", ignore_case=True),
        state=None,
    )
    dp.register_message_handler(
        delete_price_list, commands=["/удалить_прайслист"], state=None
    )
    dp.register_message_handler(
        get_name_for_deleting_price_list,
        state=FSM_Admin_delete_price_list.get_name_price_for_delete,
    )

    dp.register_message_handler(
        change_price_list, commands=["/изменить_прайслист"], state=None
    )
    dp.register_message_handler(
        change_price_list,
        Text(equals="изменить прайс-лист", ignore_case=True),
        state=None,
    )
    dp.register_message_handler(
