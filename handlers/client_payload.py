...
async def send_to_view_orders(
    orders: ScalarResult["Orders"], 
    ...
    for order in orders:
        ...
        elif order_type in kb_client.choice_order_type_to_payloading["Заказы эскизов 🎨"]:
            orders_str += \
...
async def command_send_to_view_orders_to_payloading(message: types.Message, state=FSMContext):
    if message.text in list(kb_client.choice_order_type_to_payloading.keys()):
        ...
        if unpaid_orders == []:
            ...
            await bot.send_message(message.from_id, f'📃 Ваши заказы:\n {orders_str}',
                reply_markup= kb_unpaid_orders)
            await FSM_Client_paid_order.order_name.set()
            await bot.send_message(message.from_id, MSG_WHICH_ORDER_DO_CLIENT_WANT_TO_PAY)
...
async def get_order_check_document_for_paid(message: types.Message, state=FSMContext):
    if message.content_type == 'text':
        ...
        if check_doc["result"]:
            ...
            await bot.send_message(message.from_id, f"🎉 Заказ № {order_number} оплачен! "\
                f"Статус заказа сменился с {old_state} на {STATES['paid']}")
            
            if current_order_type in kb_client.choice_order_type_to_payloading["сертификат"]:
