...
async def get_tattoo_items_and_item_command_list(message: types.Message):
    if message.text.lower() in ['татуировки', '/татуировки'] and \
        str(message.from_user.username) in ADMIN_NAMES:
        await message.reply('Какую команду хочешь выполнить?',
...
async def command_change_tattoo_item(message: types.Message):
    if message.text.lower() in ['изменить тату', '/изменить_тату'] and \
        ...
        if tattoo_items == []:
            ...
            await FSM_Admin_tattoo_item_change.tattoo_name.set()
            await message.reply('Какое тату хочешь изменить?', 
...
async def get_new_tattoo_item_state(message: types.Message, state: FSMContext):
    if message.text in kb_admin.new_tattoo_item_state.keys():
        ...
        if message.text != 'Фотография':        
            ...
            await FSM_Admin_tattoo_item_change.next() 
            await message.reply('📎 Загрузи новую фотографию тату, пожалуйста', 
                reply_markup=kb_new_state_tattoo_item[kb_admin.new_tattoo_item_state[message.text]])
...
async def set_new_tattoo_item_state(message: types.Message, state: FSMContext):
    ...
    await message.reply(f'Отлично, ты поменяла значение {new_state} у {tattoo_name}',
        reply_markup=kb_admin.kb_tattoo_items_commands)
...
async def set_new_tattoo_photo(message: types.Message, state: FSMContext):
    ...
    await update_info_in_json('tattoo_items', 'name', tattoo_name, 'photo' , message.text)
...
async def command_load_tattoo_item(message: types.Message):
    if message.text.lower() in ['добавить тату', '/добавить_тату'] and \
        str(message.from_user.username) in ADMIN_NAMES:
        await FSM_Admin_tattoo_item.tattoo_name.set()
        await message.reply('Введи название тату', 
...
async def load_tattoo_name(message: types.Message, state: FSMContext):
    if message.text in LIST_CANCEL_COMMANDS + LIST_BACK_COMMANDS + LIST_BACK_TO_HOME:
        ...
        await message.reply('А теперь загрузи фотографию тату',
...
async def load_tattoo_item_photo(message: types.Message, state: FSMContext):
    ...
    if message.content_type == 'photo':
        ...
        await FSM_Admin_tattoo_item.next()
        await message.reply('Введи примерную цену тату', 
...
async def load_tattoo_price(message: types.Message, state: FSMContext):
    if message.text == 'Другая цена':
        ...
        await FSM_Admin_tattoo_item.next()
        await message.reply(f'Тату будет ч/б или цветная?',
        ...
        await message.reply('Вы вернулись назад к выбору фото для тату.'\
...
async def load_tattoo_colored(message: types.Message, state: FSMContext):
    if message.text in kb_client.colored_tattoo_choice:
        ...
        await FSM_Admin_tattoo_item.previous()
        await message.reply('Вы вернулись назад к выбору цены. Какая примерная цена тату?',
            reply_markup = kb_admin.kb_price)
...
async def load_tattoo_note(message: types.Message, state: FSMContext):
    if message.text in LIST_CANCEL_COMMANDS:
        ...
        await FSM_Admin_tattoo_item.previous()
        await message.reply('Хорошо, укажи цвет тату',
            reply_markup = kb_client.kb_colored_tattoo_choice)
    
    elif message.text not in LIST_CANCEL_COMMANDS + LIST_BACK_COMMANDS + LIST_BACK_TO_HOME:
        async with state.proxy() as data:
        ...
        await message.reply('Готово! Вы добавили тату в таблицу tattoo_items.\n\n'\
...
async def command_get_info_all_client_tattoo(message: types.Message):
    if message.text.lower() in ['посмотреть все пользовательские тату', 
        '/посмотреть_все_пользовательские_тату'] and \
        str(message.from_user.username) in ADMIN_NAMES:
...
async def delete_info_tattoo_in_table(message: types.Message):
    if message.text.lower() in ['удалить тату', '/удалить_тату'] and \
        str(message.from_user.username) in ADMIN_NAMES:
        ...
        if tattoo_items == []:
            ...
            await bot.send_message(message.from_user.id, "Какую тату хотите удалить?",
                reply_markup= kb_tattoo_names)
...
async def delete_info_tattoo_in_table_next(message: types.Message, state: FSMContext):
    ...
    await message.reply(f'Готово! Вы удалили тату {message.text}. \n'\
...
def register_handlers_admin_tattoo_item(dp: Dispatcher):
    #---------------------------------- TATTOO ITEM COMMAND-------------------------        
    dp.register_message_handler(get_tattoo_items_and_item_command_list, commands=['татуировки'])
    dp.register_message_handler(get_tattoo_items_and_item_command_list, 
        Text(equals='Татуировки', ignore_case=True), state=None)
    
    #---------------------------------------CHANGE TATTOO------------------------------------
    dp.register_message_handler(command_change_tattoo_item, commands=['изменить_тату'])
    dp.register_message_handler(command_change_tattoo_item,
        Text(equals='изменить тату', ignore_case=True), state=None)
    dp.register_message_handler(get_tattoo_name_for_changing, 
        state=FSM_Admin_tattoo_item_change.tattoo_name)
    dp.register_message_handler(get_new_tattoo_item_state, 
        state=FSM_Admin_tattoo_item_change.new_state)
    dp.register_message_handler(set_new_tattoo_item_state, 
        state=FSM_Admin_tattoo_item_change.new_value)
    dp.register_message_handler(set_new_tattoo_photo, content_types=['photo'],
        state=FSM_Admin_tattoo_item_change.new_photo)

    #----------------------------------------ADD TATTOO-----------------------------------------
    dp.register_message_handler(command_load_tattoo_item, commands=['добавить_тату'], state=None)
    dp.register_message_handler(command_load_tattoo_item,
        Text(equals='добавить тату', ignore_case=True), state=None)
    dp.register_message_handler(load_tattoo_name, state=FSM_Admin_tattoo_item.tattoo_name)
    dp.register_message_handler(load_tattoo_item_photo, content_types= ['photo', 'text'], 
        #  types.ContentType.all(),
        state=FSM_Admin_tattoo_item.tattoo_photo)
    dp.register_message_handler(load_tattoo_price, state=FSM_Admin_tattoo_item.tattoo_price)
    dp.register_message_handler(load_tattoo_colored, state=FSM_Admin_tattoo_item.tattoo_colored)
    # dp.register_message_handler(load_tattoo_number_of_details,
    # state=FSM_Admin_tattoo_item.tattoo_number_of_details)
    dp.register_message_handler(load_tattoo_note, state=FSM_Admin_tattoo_item.tattoo_note)
    
    dp.register_message_handler(command_get_info_all_tattoo, commands=['посмотреть_все_тату'])
    dp.register_message_handler(command_get_info_all_tattoo,
        Text(equals='посмотреть все тату', ignore_case=True), state=None)
    
    dp.register_message_handler(command_get_info_all_admin_tattoo, 
        commands=['посмотреть_все_мои_тату'])
    dp.register_message_handler(command_get_info_all_admin_tattoo,
        Text(equals='посмотреть все мои тату', ignore_case=True), state=None)
    
    dp.register_message_handler(command_get_info_admin_tattoo, commands=['посмотреть_мои_тату'])
    dp.register_message_handler(command_get_info_admin_tattoo,
        Text(equals='посмотреть мои тату', ignore_case=True), state=None)
    
    dp.register_message_handler(command_get_info_all_client_tattoo,
        commands=['посмотреть_все_пользовательские_тату'])
    dp.register_message_handler(command_get_info_all_client_tattoo,
        Text(equals='посмотреть все пользовательские тату', ignore_case=True), state=None)
    
    dp.register_message_handler(command_get_info_client_tattoo,
        commands=['посмотреть_пользовательские_тату'])
    dp.register_message_handler(command_get_info_client_tattoo,
        Text(equals='посмотреть пользовательские тату', ignore_case=True), state=None)
    dp.register_message_handler(get_tattoo_name_for_tattoo_info,
    ...
    dp.register_message_handler(command_get_info_tattoo,
        Text(equals='посмотреть тату', ignore_case=True), state=None)
    dp.register_message_handler(get_tattoo_name_for_info, 
        state=FSM_Admin_get_info_tattoo_item.tattoo_name)
    
    dp.register_message_handler(delete_info_tattoo_in_table, commands=['удалить_тату'])
    dp.register_message_handler(delete_info_tattoo_in_table,
        Text(equals='удалить тату', ignore_case=True), state=None)
    dp.register_message_handler(delete_info_tattoo_in_table_next,
