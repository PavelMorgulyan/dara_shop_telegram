...
async def get_cert_command_list(message: types.Message):
    if message.text.lower() == 'сертификат' and \
        str(message.from_user.username) in ADMIN_NAMES:
        await message.reply('Какую команду по сертификатам хочешь выполнить?',
...
async def command_load_сert_item(message: types.Message):
    if message.text.lower() == 'добавить заказ на сертификат' and \
        str(message.from_user.username) in ADMIN_NAMES:
        await FSM_Admin_сert_item.сert_price.set()
        await message.reply('На какую цену хотите сертификат?',
...
async def admin_process_successful_cert_payment(message: types.Message, state=FSMContext):
    if message.text == kb_client.yes_str:
        async with state.proxy() as data: # type: ignore
            ...
            await FSM_Admin_сert_item.next()
            await message.reply(f'Хочешь приложить чек перевода?', reply_markup=kb_client.kb_yes_no) 
...
async def cert_load_user_name(message: types.Message, state: FSMContext):
    ...
    if message.text != list(user[0])[0]:
        ...
        await FSM_Admin_cert_username_info.next()
        await message.reply(f'Это пользователь под ником {user_telegram}?',
            reply_markup= kb_client.kb_yes_no)
...
async def cert_answer_user_name(message: types.Message, state: FSMContext):
    if message.text == kb_client.yes_str:
        async with state.proxy() as data:
            ...
            await update_info('cert_orders',  'order_number', order_number, 'username', username)
            
            await bot.send_message(message.from_user.id, f'Хорошо, твой заказ под номером {order_number} '\
...
async def cert_load_phone(message: types.Message, state: FSMContext):
    ...
    result = re.match(r'^(\+7|7|8)?[\s\-]?\(?[489][0-9]{2}\)?[\s\-]?'\
        r'[0-9]{3}[\s\-]?[0-9]{2}[\s\-]?[0-9]{2}$', number)
    ...
    if not result:
        ...
        await bot.send_message(message.from_user.id, f'Хорошо, твой заказ под номером {tattoo_order_number}'\
...
async def command_get_info_сert_orders(message: types.Message): 
    if message.text.lower() in ['посмотреть заказанные сертификаты', '/посмотреть_заказанные_сертификаты']\
        ...
        if orders_into_table == []:
            ...
            for ret in orders_into_table:
                try:
                    username_phone = await get_info_many_from_table('clients', 'username', ret[0])
                ...
                # else:
                #    number_deleted_order += 1
            await bot.send_message(message.from_user.id, f'Всего заказов: {number_deleted_order}',
                reply_markup= kb_admin.kb_main)
            
def register_handlers_admin_cert(dp: Dispatcher):            
    #-------------------------------------------------------CREATE CERT ORDER------------------------------------------------------
    dp.register_message_handler(get_cert_command_list, 
        commands='сертификат', state=None)
    dp.register_message_handler(get_cert_command_list, 
        Text(equals='сертификат', ignore_case=True), state=None)

    dp.register_message_handler(command_load_сert_item, 
        commands='добавить_заказ_на_сертификат', state=None)
    dp.register_message_handler(command_load_сert_item, 
        Text(equals='добавить заказ на сертификат', ignore_case=True), state=None)
    dp.register_message_handler(load_сert_price,
        state=FSM_Admin_сert_item.сert_price)
    dp.register_message_handler(load_сert_other_price,
        state=FSM_Admin_сert_item.сert_other_price)

    dp.register_message_handler(admin_process_successful_cert_payment, 
        state=FSM_Admin_сert_item.сert_payment_choise) # Пользователь оплатил сертфикат? Да / нет
    dp.register_message_handler(get_check_answer_cert_from_admin, 
        state=FSM_Admin_сert_item.cert_get_check_answer) # Хочешь приложить чек перевода?  Да / нет
    dp.register_message_handler(get_check_document_cert_from_admin, 
        state=FSM_Admin_сert_item.cert_get_check_document)
    
    dp.register_message_handler(cert_load_user_name,
        state=FSM_Admin_cert_username_info.get_user_name) # Приложи документ или фотографию чека
    dp.register_message_handler(cert_answer_user_name,
        state=FSM_Admin_cert_username_info.user_name_answer)
    dp.register_message_handler(cert_load_telegram,
        state=FSM_Admin_cert_username_info.telegram) # добавляет всю инфу про пользователя
    dp.register_message_handler(cert_load_phone,
        state=FSM_Admin_cert_username_info.phone) # добавляет всю инфу про пользователя
    
    #-------------------------------------------------------COMMANDS CERT ORDER------------------------------------------------------

    dp.register_message_handler(command_get_info_сert_orders,
        commands='посмотреть_заказанные_сертификаты', state=None)
    dp.register_message_handler(command_get_info_сert_orders, 
        Text(equals='посмотреть заказанные сертификаты', ignore_case=True), state=None)