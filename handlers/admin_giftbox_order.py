...
async def send_to_view_giftbox_order(message:types.Message, orders: ScalarResult["Orders"]):
    if orders == []:
        ...
        for order in orders:
            with Session(engine) as session:
                user = session.scalars(select(User).where(User.telegram_id == order.user_id)).all()
                
            if user != []:
                with Session(engine) as session:
                    phone = session.scalars(select(User)
                        .where(User.telegram_id == order.user_id)).one().phone
            else:
                phone = 'Нет номера'
    ...
    if message.text in kb_admin.statuses_order_lst:
        with Session(engine) as session:
            orders = session.scalars(select(Orders)
                ...
                .where(Orders.order_state == message.text)).all()
    ...
    if message.text.lower() in [ '/поменять_статус_гифтбокс_заказа', 'поменять статус гифтбокс заказа'] \
        ...
        with Session(engine) as session:
            giftbox_orders = session.scalars(select(Orders)
                .where(Orders.order_type == 'гифтбокс')).all()
        ...
        if giftbox_orders is None or giftbox_orders == []:
            ...
            for order in giftbox_orders:
                ...
                kb_giftbox_numbers.add(KeyboardButton(
...
async def set_new_state_giftbox_order(message: types.Message, state: FSMContext): 
    ...
    with Session(engine) as session:
        order = session.scalars(select(Orders)
            .where(Orders.order_number == giftbox_number)).one()
