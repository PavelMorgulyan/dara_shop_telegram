...
async def start_create_new_tattoo_order(message: types.Message):
    ...
    if user == []:
        ...
        # Если пользователь в бане, то не пускаем дальше и просто выдаем сообщение
        if user[0].status == clients_status['banned']:
            await bot.send_message(message.from_id, MSG_BANNED_CLIENT)
        else:
            # -> get_client_choice_main_or_temporary_tattoo
            await FSM_Client_tattoo_order.client_choice_main_or_temporary_tattoo.set()
            await bot.send_message(message.from_id, MSG_START_CLIENT_TATTOO_ORDER) 
...
async def get_client_choice_main_or_temporary_tattoo(
    ...
    if message.text in list(kb_client.choice_main_or_temporary_tattoo.values()):
        ...
        if orders == []:
            ...
            with Session(engine) as session:
                sizes = session.scalars(
                    select(OrderPriceList).where(
                        OrderPriceList.type == message.text[:-2].lower()
                    )
                ).all()
            ...
            async with state.proxy() as data:
                ...
                data[
                    "load_tattoo_desc"
                ] = False  # для меню загрузки описания эскиза тату - для кнопки "Назад"
                ...
                data[
                    "tattoo_body_place"
                ] = "Без места для тату"  # изначальное место для тату в заказе
                data[
                    "tattoo_details_number"
                ] = 0  # изначальное количество деталей тату в заказе
...
async def load_tattoo_order_photo(message: types.Message, state: FSMContext):
    ...
    if message.content_type == "text":
        ...
        # Хочу эскиз по моему описанию 💬
            ...
            await bot.send_message(
                message.from_id, f"{MSG_GET_DESCRIPTION_TATTOO_FROM_CLIENT}"
            )
            await bot.send_message(
                message.from_id, f"{MSG_GET_DESCRIPTION_TATTOO_FROM_CLIENT_CONCEPTS}"
            )
            """ 
        ...
        # переходим сюда, если клиент выбрал эскиз из списка
            ...
            await bot.send_message(
                message.from_id, "📷 Отлично, ты выбрал(а) эскиз для своего тату!"
            )
            await bot.send_message(
                message.from_id,
        ...
        # Да, отличное изображение, хочу такой эскиз ☘️
            ...
            await bot.send_message(
                message.from_id,
                f"{MSG_CLIENT_SUCCESS_CHOICE_PHOTO}" f"{MSG_CLIENT_CHOICE_TATTOO_NAME}",
                reply_markup=kb_client.kb_back_cancel,
            )
...
async def change_menu_tattoo_from_galery(message: types.Message, state: FSMContext):
    ...
    if any(text in message.text for text in LIST_CANCEL_COMMANDS + LIST_BACK_TO_HOME):
        ...
        async with state.proxy() as data:
            ...
            data["next_menu_another_size"], data["next_menu_detailed_number"] = (
                False,
                False,
            )
        ...
        if next_menu_another_size:
            ...
            next_menu_detailed_number
            or next_menu_change_color
            or next_menu_size
            or menu_change_name
        ):
            await bot.send_message(
                message.from_id,
                f"{MSG_CLIENT_GO_BACK}{CLIENT_WANT_TO_CHANGE_MORE}",
                reply_markup=kb_client.kb_tattoo_from_galery_change_options,
            )
            ...
            await bot.send_message(
                message.from_id,
                f"{MSG_CLIENT_GO_BACK}{MSG_GET_PHOTO_FROM_USER}",
                reply_markup=kb_client.kb_no_photo_in_tattoo_order,
            )
    ...
    # меняем имя тату
        ...
        message.text
        == kb_client.tattoo_from_galery_change_options[
            "client_want_to_change_tattoo_name"
        ]
    ):
        async with state.proxy() as data:
        ...
        await bot.send_message(
            message.from_id,
            f"📏 Хорошо, давай поменяем имя тату\n\n" f"{MSG_CLIENT_CHOICE_TATTOO_NAME}",
            reply_markup=kb_client.kb_back_cancel,
        )
    ...
    # меняем цвет тату
        ...
        message.text
        == kb_client.tattoo_from_galery_change_options[
            "client_want_to_change_tattoo_color"
        ]
    ):
        async with state.proxy() as data:
    ...
    # меняем детали в тату
        ...
        message.text
        == kb_client.tattoo_from_galery_change_options[
            "client_want_to_change_tattoo_details"
        ]
    ):
        async with state.proxy() as data:
    ...
    # ничего не хочу менять -> get_choice_tattoo_place
        ...
        await bot.send_message(
            message.from_id,
            f"{MSG_DO_CLIENT_WANT_TO_ADD_BODYPLACE}",
...
async def get_choice_colored_or_not(message: types.Message, state: FSMContext):
    if any(text in message.text for text in kb_client.colored_tattoo_choice):
        ...
        await bot.send_message(
            message.from_id,
            f"🍃 Тату будет {message.text.split()[0].lower()}.\n"
            "❕ Цвета конкретных деталей вашего эскиза вы можете "
            "описать в комментарии к тату (далее в заявке)",
        )

        await bot.send_message(
            message.from_id,
            MSG_DO_CLIENT_WANT_TO_ADD_BODYPLACE,
...
async def view_schedule_to_client(message: types.Message, state: FSMContext):
    ...
    if schedule == []:
        await bot.send_message(
            message.from_id, MSG_TO_NO_SCHEDULE, reply_markup=kb_client.kb_next_action
        )
    else:
        ...
        with Session(engine) as session:
            schedule_photo = session.scalars(
                select(SchedulePhoto).where(
                    SchedulePhoto.name == datetime.strftime(datetime.now(), "%m %Y")
                )
            ).all()
        ...
        async with state.proxy() as data:
            ...
            for date in schedule:
                if date.start_datetime > datetime.now():
                    ...
                    item_in_kb = (
                        f"{month} {date.start_datetime.strftime('%d/%m/%Y c %H:%M')}"
            ...
            kb_schedule.add(KeyboardButton(kb_client.order_without_schedule_event))
        ...
        await bot.send_message(
            message.from_user.id, MSG_TO_GET_SCHEDULE, reply_markup=kb_schedule
...
async def get_choice_tattoo_place(message: types.Message, state: FSMContext):
    ...
    if message.text == kb_client.choice_place_tattoo["client_know_place"]:
        ...
        await bot.send_message(
            message.from_id,
            "🗾 Выберите место для своего тату, пожалуйста",
            reply_markup=kb_client.kb_place_for_tattoo,
        )
    #
    # "Нет, я пока не знаю, где будет мое тату"
    elif message.text in [
        kb_client.choice_place_tattoo["client_has_no_idea_for_place"],
        kb_client.tattoo_body_places[-1],
        ...
        await bot.send_message(
            message.from_id, MSG_TATTOO_ORDER_CLIENT_CHOICE_BODY_LATER
        )
    ...
    # "Да, хочу отправить фото/видео "
        ...
        message.text
        == kb_client.choice_get_photo_for_place_tattoo["client_want_to_get_place"]
    ):
    ...
    # "Нет, не хочу"
        ...
        message.text
        == kb_client.choice_get_photo_for_place_tattoo["client_dont_want_to_get_place"]
    ):
        await bot.send_message(
            message.from_id, "❕ Не проблема, добавить фото/видео для тату можно позже."
        )
        async with state.proxy() as data:
            tattoo_from_galery = data["tattoo_from_galery"]
            data["client_add_tattoo_place_photo"] = False
        ...
        if tattoo_body_place == kb_client.tattoo_body_places[-2]: # Другое место 🙅‍♂️
...
async def get_photo_place_for_tattoo(message: types.Message, state: FSMContext):
    ...
    if message.content_type == "text":
        ...
        if message.text == kb_client.yes_str:
            ...
            await bot.send_message(
                message.from_id,
                f"{MSG_CLIENT_GO_BACK}" "🗾 Выбери место для своего тату",
                reply_markup=kb_client.kb_place_for_tattoo,
            )
        else:
            await bot.send_message(
                message.from_id, f"📎 Пожалуйста, Отправьте фотографию через файлы"
            )
...
async def get_size_tattoo_from_galery(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == kb_client.another_size:
        ...
        async with state.proxy() as data:
            ...
            data["client_add_tattoo_place_photo"], data["next_menu_another_size"] = (
                False,
                False,
            )
...
# choice_tattoo_place ->-> 
#   choice_tattoo_order_date_and_time_meeting -> 
#       next_choice_tattoo_order_date_meeting
#
# get_size_tattoo_from_galery -> choice_tattoo_order_date_and_time_meeting ->-> load_datemeeting
async def choice_tattoo_order_date_and_time_meeting(
    message: types.Message, state: FSMContext
):
    ...
    if any(text in message.text for text in LIST_CANCEL_COMMANDS):
        ...
        async with state.proxy() as data:
            ...
            if (
                message.text in schedule_lst
            ):  # если выбираем конкретную дату из календаря
                ...
                with Session(engine) as session:
                    schedule_event = session.scalars(
                        ...
                        .where(
                            ...
                            == datetime.strptime(
                                f"{message.text.split()[1]} {message.text.split()[3]}",
                                "%d/%m/%Y %H:%M",
                            )
                        )
                        .where(
                            ScheduleCalendar.end_datetime
                            == datetime.strptime(
                                f"{message.text.split()[1]} {message.text.split()[5]}",
                                "%d/%m/%Y %H:%M",
                            )
                        )
                    ).one()
                data["date_meeting"] = schedule_event.start_datetime
                data["start_date_time"] = schedule_event.start_datetime.strftime(
                    "%H:%M"
                )
    ...
    # TODO добавить обработку фразы kb_client.order_without_schedule_event
    # "Выберу дату вместе с админом 🤔"


async def next_choice_tattoo_order_date_meeting(
    message: types.Message, state: FSMContext
):
    async with state.proxy() as data:
    ...
    for _ in range(2):
        await FSM_Client_tattoo_order.next()  # -> load_tattoo_order_note
    await bot.send_message(
        message.from_id, f"📅 Прекрасно, дата встречи теперь {message.text}!"
    )
...
@dp.callback_query_handler(
    dialog_cal_callback.filter(), state=FSM_Client_tattoo_order.date_meeting
)
async def load_datemeeting(
    ...
    if selected:
        ...
        if date > datetime.now():
            ...
            await bot.send_message(
                user_id,
                f"{MSG_NOT_CORRECT_DATE_NOW_LESS_CHOICEN}"
...
@dp.callback_query_handler(
    full_timep_callback.filter(), state=FSM_Client_tattoo_order.date_time
)
async def process_hour_timepicker(
    callback_query: CallbackQuery, callback_data: dict, state: FSMContext
):
    ...
    if r.selected:
        ...
        async with state.proxy() as data:
            ...
            data["end_date_time"] = str(int(r.time.strftime("%H")) + 3) + str(
                r.time.strftime(":%M")
            )
...
async def load_tattoo_order_note(message: types.Message, state: FSMContext):
    if any(text in message.text for text in LIST_CANCEL_COMMANDS):
            ...
            if tattoo_type == kb_admin.price_lst_types["constant_tattoo"]:
                if data["event_type_creation"] == "from_schedule":
                    ...
                    await bot.send_message(
                        message.from_id,
                        f"{MSG_CLIENT_GO_BACK}"
                        "❔ Какую дату хотите выбрать из предложенных?",
                    )
                    ...
                    await bot.send_message(
                        message.from_id,
                        f"{MSG_CLIENT_GO_BACK}" "🕒 введите удобное для вас время.",
                        reply_markup=await FullTimePicker().start_picker(),
                    )
        ...
        async with state.proxy() as data:
            if message.text in kb_client.no_tattoo_note_from_client: #  Мне нечего добавить 🙅‍♂️
                data["tattoo_note"] = "Без описания тату"
                await bot.send_message(
                    message.from_id,
                    f"😓 Оставим пока заказ без вашего описания тату.",
                )
            ...
            price = (
                Session(engine)
                .scalars(
                    select(OrderPriceList)
                    .where(OrderPriceList.min_size == min_size)
                    .where(OrderPriceList.max_size == max_size)
                )
                .all()
            )
        ...
        await FSM_Client_tattoo_order.next() #-> choiсe_tattoo_order_desctiption
        await bot.send_message(
            message.from_id,
            f"{MSG_CLIENT_WANT_TO_FILL_ORDER_NOTE}",
            reply_markup=kb_client.kb_yes_no,
        )
...
async def choiсe_tattoo_order_desctiption(message: types.Message, state: FSMContext):
    if message.text == kb_client.yes_str:
        ...
        await bot.send_message(
            ...
            " Вы можете написать что-угодно по заказу, "
            "данный комментарий обязательно будет учитываться при рассмотрении заявки",
            reply_markup=kb_client.kb_back_cancel,
        )
    ...
    for order in orders:
        ...
        if order.order_type == kb_admin.price_lst_types["constant_tattoo"]:
            ...
            if order.schedule_id is not None:
                ...
                msg += f"🕒 Дата и время встречи не выбраны - свободных ячеек в календаре нет.\n"
        msg += (
        ...
        with Session(engine) as session:
            check_document_lst = session.scalars(
                select(CheckDocument).where(
                    CheckDocument.order_number == order.order_number
                )
            ).all()
        if check_document_lst != []:
            for doc in check_document_lst:
                if doc.doc in [MSG_CHECK_NOT_ADDED, "-", None, ""]:
                    msg += f"{MSG_CHECK_NOT_ADDED}\n"
                else:
                    msg += "🍀 Чек добавлен\n"
        else:
            msg += f"{MSG_CHECK_NOT_ADDED}\n"
        ...
        tattoo_photos = (
            Session(engine)
            .scalars(
                select(OrderPhoto).where(OrderPhoto.order_number == order.order_number)
            )
            .all()
        )
        ...
        for photo_bodyplace in body_photos:
            media.append(
                types.InputMediaPhoto(
                    photo_bodyplace.photo, f"👤 Местоположение тату: {order.bodyplace}"
                )
            )
...
async def get_clients_tattoo_order(message: types.Message):
    with Session(engine) as session:
        orders = session.scalars(
            select(Orders)
            .where(Orders.user_id == message.from_id)
            .where(Orders.order_type.in_(["переводное тату", "постоянное тату"]))
        ).all()
    ...
    if orders == []:
        ...
        for order in orders:
            kb.add(
                KeyboardButton(
                    f'Тату заказ №{order.order_number} "{order.order_name}" {order.order_state}'
                )
            )
...
async def get_tattoo_order_number_to_view(message: types.Message, state: FSMContext):
    ...
    if message.text in kb_answer_lst:
        with Session(engine) as session:
            orders = session.scalars(
                select(Orders).where(Orders.order_number == message.text.split()[2][1:])
            ).all()
...
async def get_order_id_to_add_new_photo(message: types.Message, state: FSMContext):
    ...
    kb_orders_lst = []
    for order in orders:
        creation_date = order.creation_date.strftime(
            "%d/%m/%Y"
        )  # TODO нужно получить %d/%m/%Y
        order_number = order.order_number
...
async def get_photo_type(message: types.Message, state: FSMContext):
    ...
    if message.text in list(kb_client.client_choice_add_photo_type.values()):
        ...
        await bot.send_message(
            message.from_id, msg, reply_markup=kb_client.kb_back_cancel
        )
        ...
        await bot.send_message(
            message.from_id, MSG_PLS_SEND_TATTOO_PHOTO_OR_CANCEL_ACTION
        )
    ...
    if message.content_type == "photo":
        ...
        with Session(engine) as session:
            order = session.scalars(
                select(Orders).where(Orders.order_number == tattoo_order_number)
            ).one()
        ...
        with Session(engine) as session:
            ...
            order.tattoo_place_video_note.append(
                TattooPlaceVideoNote(
                    order_number=tattoo_order_number,
                    # order_id=order_id,
                    video=message.video_note.file_id,
...
def register_handlers_client_tattoo_order(dp: Dispatcher):
    dp.register_message_handler(
        start_create_new_tattoo_order,
        Text(equals=kb_client.client_main["client_want_tattoo"], ignore_case=True),
        state=None,
    )

    dp.register_message_handler(
        start_create_new_tattoo_order, commands=["get_tattoo"], state=None
    )
    dp.register_message_handler(
        get_client_choice_main_or_temporary_tattoo,
    ...
    dp.register_message_handler(
        change_menu_tattoo_from_galery,
        state=FSM_Client_tattoo_order.change_tattoo_from_galery,
    )
    dp.register_message_handler(
        load_tattoo_order_name, state=FSM_Client_tattoo_order.tattoo_name
    )
    dp.register_message_handler(
        load_tattoo_order_size, state=FSM_Client_tattoo_order.tattoo_size
    )
    dp.register_message_handler(
        get_choice_colored_or_not, state=FSM_Client_tattoo_order.choice_colored_or_not
    )
    # dp.register_message_handler(get_choice_number_of_details,
    # state=FSM_Client_tattoo_order.choice_number_of_details)
    dp.register_message_handler(
        get_choice_tattoo_place, state=FSM_Client_tattoo_order.get_choice_tattoo_place
    )
    dp.register_message_handler(
        get_photo_place_for_tattoo,
    ...
    dp.register_message_handler(
        load_tattoo_order_note, state=FSM_Client_tattoo_order.tattoo_note
    )
    dp.register_message_handler(
    ...
    # Посмотреть мои тату заказы 🕸
    dp.register_message_handler(
        get_clients_tattoo_order,
        Text(
            equals=kb_client.choice_order_view["client_watch_tattoo_order"],
            ignore_case=True,
        ),
        state=None,
    )
    dp.register_message_handler(
        get_clients_tattoo_order, commands=["посмотреть_мои_тату_заказы"], state=None
    )
    dp.register_message_handler(
        get_tattoo_order_number_to_view,
        state=FSM_Client_send_to_client_view_tattoo_order.get_order_number,
    )

    dp.register_message_handler(
        get_new_photo_to_tattoo_order,
        Text(
            equals=kb_client.choice_order_view["client_add_photo_to_tattoo_order"],
            ignore_case=True,
        ),
        state=None,
    )
    dp.register_message_handler(
        get_new_photo_to_tattoo_order,
        commands=["добавить_фото_в_тату_заказ"],
        state=None,
    )

    dp.register_message_handler(
        get_order_id_to_add_new_photo,
        state=FSM_Client_get_new_photo_to_tattoo_order.get_order_id,
    )
    dp.register_message_handler(
        get_photo_type, state=FSM_Client_get_new_photo_to_tattoo_order.get_photo_type
    )
    dp.register_message_handler(
        get_new_photo,
