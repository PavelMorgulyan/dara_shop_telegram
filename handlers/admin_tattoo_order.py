    ...
    if tattoo_orders == []:
        ...
        for order in tattoo_orders:
            ...
            if check_document not in [[], None]:
                # TODO сделать вывод чеков
                for doc in check_document:
                    if doc is not None and doc.doc is not None:
                        try:
                            await bot.send_document(
                                message.from_user.id, doc.doc, MSG_SUCCESS_GETTING_CHECK
                            )
                        except:
                        ...
                        msg += MSG_CHECK_NOT_ADDED
            else:
                msg += MSG_CHECK_NOT_ADDED
            await bot.send_message(message.from_id, msg)


async def send_to_view_tattoo_order_in_table(
    message: types.Message, tattoo_orders: ScalarResult["Orders"]):
    if tattoo_orders == []:
        await bot.send_message(
            message.from_id,
        ...
        for number, order in enumerate(tattoo_orders):
            with Session(engine) as session:
                schedule_lst = session.scalars(
                    select(ScheduleCalendarItems).where(
                        ScheduleCalendarItems.order_number == order.order_number
            ...
            if order.order_type == kb_admin.price_lst_types["constant_tattoo"]:
                if schedule_lst not in [None, []]:
                    ...
                    for schedule in schedule_lst:
                        ...
                        sessions += f"{number + 1}) {start_time} - {end_time}\n"
                else:
                    sessions = (f"---")
            ...
            if data['mode'] == 'pc':
                ...
                msg += (
                    ...
                    f"- {headers[1]}: {order.order_number}\n"
                    f"- {headers[2]}: {order.order_type.split()[0]}\n"
                    f"- {headers[3]}: {order.order_name}\n"
                    f"- {headers[4]}: {order.tattoo_size}\n"
                    f"- {headers[5]}: {order.colored}\n"
                    f"- {headers[6]}: {order.bodyplace[:-2]}\n"
                    f"- {headers[7]}: {order.price}\n"
                    f"- {headers[8]}: {order.username}\n"
                    f"- {headers[9]}: {order.order_state}\n"
                    f"- {headers[10]}:\n{sessions}\n\n"
                )
        if data['mode'] == 'pc':
            await bot.send_message(
...
async def get_new_status_for_tattoo_order(message: types.Message, state: FSMContext):
    ...
    if message.text in tattoo_order_numbers_lst:
        ...
        await bot.send_message(
            message.from_id, "❔ Какой статус выставляем?",
            reply_markup=kb_new_status,
        )
...
async def complete_new_status_for_tattoo_order(
    ...
    if message.text in status_distribution[kb_admin.price_lst_types["constant_tattoo"]][current_order_status]:
        ...
        await bot.send_message(
            message.from_id,
            f'🎉 Готово!'
            f'Вы обновили статус заказа {order_number} на "{new_state}"',
        )
        
        if new_state == STATES['paid']:
            await FSM_Admin_tattoo_order_change_status.next()  # -> get_answer_for_getting_check_document
            await message.reply(
                MSG_GET_CHECK, reply_markup=kb_client.kb_yes_no
            )
...
async def get_answer_for_getting_check_document(
    ...
    if message.text == kb_client.yes_str:
        ...
        await message.reply(
            f"❌ На этот вопрос можно ответит только 'Да' или 'Нет'."
...
async def command_command_change_info_tattoo_order(message: types.Message):
    if (
        ...
        if orders == []:
            ...
            await message.reply(
                "❔ У какого заказа изменить какую-либо информацию?",
                reply_markup=kb_tattoo_order_numbers,
            )
...
async def get_tattoo_type(message: types.Message, state: FSMContext):
    if message.text in list(kb_client.choice_main_or_temporary_tattoo.values()):
        ...
        await bot.send_message(
            message.from_id,
            "❔ Это тату из твоей галереи или нет?",
            reply_markup=kb_client.kb_yes_no,
        )
...
async def choice_tattoo_order_admin(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        tattoo_names_lst = data['tattoo_names_lst']
    ...
    if message.text == kb_client.yes_str:
        ...
        for tattoo in admin_tattoo_items:
            ...
            """ media = []
            tattoo_photos = session.scalars(
                select(TattooItemPhoto)
                .where(
                    TattooItemPhoto.tattoo_item_name == tattoo.name
                )
            ).all()
            
            for photo in tattoo_photos:
        ...
        await bot.send_message(
            message.from_id,
            "❔ Какое тату выбираешь?",
            reply_markup=kb_tattoo_items_for_order.add(KeyboardButton(LIST_CANCEL_COMMANDS[-1])),
        )
        async with state.proxy() as data:
            data['tattoo_names_lst'] = tattoo_names_lst

    elif message.text in tattoo_names_lst:
        with Session(engine) as session:
        ...
        await bot.send_message(
            message.from_id,
            "💬 Введите размер тату (в см). Размер не может быть больше 150 см2 и меньше 0",
        ...
        await bot.send_message(
            message.from_id,
            "💬 Хорошо, это будет новое тату. Давай определимся с названием. "
...
async def load_tattoo_order_name(message: types.Message, state: FSMContext):
    if message.text in LIST_CANCEL_COMMANDS + LIST_BACK_TO_HOME:
        ...
        await bot.send_message(
            message.from_id, "➡️ Оставим пока данный заказ без эскиза"
        )
        ...
        await bot.send_message(
            message.from_id,
            f"❔ Добавить фотографию эскиза тату?",
            reply_markup=kb_client.kb_yes_no,
        )
...
async def load_tattoo_order_photo(message: types.Message, state: FSMContext):
    if message.content_type == "text":
        ...
        #'Закончить с добавлением изображений ➡️'
            ...
            await bot.send_message(
                message.from_id,
                "❕ С фотографиями для эскиза закончили.",
            )
            await bot.send_message(
                message.from_id,
        ...
        if tattoo_order_photo_counter != data["tattoo_order_photo_counter"]:
            ...
            await bot.send_message(
                message.from_id,
                "📷 Выбрана фотография эскиза для тату!",
            )
            await bot.send_message(
                message.from_id,
                "❔ Добавить еще фото/картинку?",
...
async def load_tattoo_order_size(message: types.Message, state: FSMContext):
    if message.text in list(kb_client.size_dict.values()):
        ...
        if tattoo_from_galery and tattoo_type.lower() == "постоянное":
            ...
            await message.reply(
                "💬 Хорошо, а теперь напишите дату или даты тату заказа.\n\n"
                "❔ Выбрать дату заказа из календаря, или добавить новую дату в календарь и заказ?",
...
async def get_tattoo_color(message: types.Message, state: FSMContext):
    if any(text in message.text for text in kb_client.colored_tattoo_choice):
        ...
        await bot.send_message(
            message.from_id, f"🍃 Тату будет {message.text.split()[0].lower()}"
        )
        ...
        if tattoo_type.lower() == "постоянное":
            ...
            await bot.send_message(
                message.from_id,
                "💬 А теперь введите что-нибудь об этом тату."
                "Так в заказе будет определен параметр \"описание тату\"",
                reply_markup=kb_client.kb_cancel,
            )
...
async def load_new_tattoo_order_date_from_schedule(
    ...
    if message.text in date_lst_to_view.split('\n'):
        ...
        for _ in range(4):
            await FSM_Admin_tattoo_order.next() #-> load_tattoo_order_note
        await message.reply(
            f"⏱ Выбрана дата {message.text.split()[2]} и время {message.text.split()[4]}.\n\n"
            "💬 А теперь введите что-нибудь о тату"
        )
...
async def process_hour_timepicker_start(
    ...
    if r.selected:
        await callback_query.message.edit_text(
            f'🕒 Время начала тату заказа в {r.time.strftime("%H:%M")} ',
        )
        # await callback_query.message.delete_reply_markup()
        username_id = 0
        async with state.proxy() as data:
...
async def process_hour_timepicker_end(
    ...
    if r.selected:
        ...
        await bot.send_message(
            username_id,
            (
                '💬 А теперь введи что-нибудь об этом тату.'
                ' Так ты добавишь параметр "описание тату"'
            ),
            reply_markup=kb_client.kb_cancel,
        )
...
async def choiсe_tattoo_order_desctiption(message: types.Message, state: FSMContext):
    ...
    if message.text == kb_client.yes_str:
        ...
        await message.reply(
            "➡️ Хорошо, тогда будем думать над деталями тату потом.\n\n"
            "💬 Напиши примерную цену тату заказа. Выбери из представленного списка",
            reply_markup=kb_admin.kb_price,
        )
    else:
        await bot.send_message(
...
async def get_tattoo_order_state(message: types.Message, state: FSMContext):
    ...
    if message.text == kb_client.yes_str:
        ...
        for _ in range(2):
            await FSM_Admin_tattoo_order.next() # -> get_tattoo_order_check_next
        ...
        await bot.send_message(
            message.from_id,
            "❌ На данный вопрос можно ответить только \"Да\" или \"Нет\".\n\n"
            "💬 Введи ответ корректно."
        )
