...
async def command_client_schedule_event(message: types.Message):# state: FSMContext)
    ...
    if orders == []:
        ...
        await bot.send_message(message.from_id, MSG_WHAT_CLIENT_WANT_TO_SEE_IN_SCHEDULE, 
...
async def command_client_create_new_schedule_event(message: types.Message):
    ...
    # Если заказ оплачен и имеет статус "в работе", а сеанс уже прошел
        ...
        await bot.send_message(message.from_id,
            ...
            'Список заказов находится у вас в списке кнопок.', reply_markup = kb)
    ...
    if message.text in kb_msg:
        ...
        for event in open_schedule_events:
            ...
            await bot.send_message(message.from_id,
                ...
                'Список свободных дат появился выше.', reply_markup= kb)
...
async def get_schedule_number_to_create_new_event_in_order(message: types.Message, state: FSMContext):
    ...
    if message.text in open_schedule_events_lst:
        with Session(engine) as session:
            ...
            session.commit()
        await bot.send_message(message.from_id, "☘️ Отлично, в заказе появился новый сеанс!\n"\
...
async def command_client_change_schedule_event(message: types.Message):
    with Session(engine) as session:
        orders = session.scalars(select(Orders)
            ...
            .where(Orders.order_type.in_([kb_admin.price_lst_types['constant_tattoo']]))
            .where(Orders.user_id == message.from_id)
            .where(Orders.order_state.in_([STATES["processed"], STATES['paid'], STATES['in_work']]))
                
            # .where(Orders.order_state.in_([STATES['in_work'],
            # STATES['open'], STATES['complete'], STATES['paid']]))
            ).all()
    await bot.send_message(message.from_id, MSG_QUESTION_CLIENT_ABOUT_CHANGING_SCHEDULE_EVENT)
    #-> get_client_answer_to_change_schedule
    await FSM_Client_client_change_schedule_event.client_get_answer_yes_no.set() 
    await bot.send_message(message.from_id, "❔ Хотите продолжить изменение даты сеанса?", 
        reply_markup=kb_client.kb_yes_no)


async def get_client_answer_to_change_schedule(message: types.Message, state: FSMContext):
    if message.text == kb_client.yes_str:
        ...
        for order in orders:
            with Session(engine) as session:
                event = session.scalars(select(ScheduleCalendar)
                    .where(ScheduleCalendar.id == order.schedule_id)).one()
                
            event_time = f"Cеанс {event.start_datetime.strftime('%H:%M')} - "\
        ...
        await bot.send_message(message.from_id, "❔ Какой сеанс хотите перенести?", reply_markup= kb)
...
async def get_event_schedule_date(message: types.Message, state: FSMContext):
    ...
    if message.text in event_time_lst:
        with Session(engine) as session:
            opened_schedule_events = session.scalars(select(ScheduleCalendar)
                .where(ScheduleCalendar.start_datetime > datetime.now())
                .where(ScheduleCalendar.end_datetime > datetime.now())
        ...
        for event in opened_schedule_events:
            free_event_time = f"Cеанс {event.start_datetime.strftime('%H:%M')} - "\
                f"{event.end_datetime.strftime('%H:%M %d/%m/%Y')} 🕒"
        ...
        kb.add(kb_client.back_btn).add(kb_client.cancel_btn)
        await bot.send_message(message.from_id, MSG_CLIENT_CHOOSE_CHANGING_EVENT,
            reply_markup= kb)
...
async def get_new_event_schedule_date(message: types.Message, state: FSMContext):
    ...
    if message.text in free_event_time_lst:
        with Session(engine) as session:
            order = session.scalars(select(Orders)
                .where(Orders.order_number == 
                    order_number_with_date_dict[event_time_lst.index(message.text)]['order_number'])).one()
            ...
            new_schedule = session.get(ScheduleCalendar, 
            ...
            new_time =  f"{new_schedule.start_datetime.strftime('%H:%M')} - "\
        ...
        async with state.proxy() as data:
            kb_get_client_answer_to_change_schedule = data['kb_get_client_answer_to_change_schedule']
            
        await bot.send_message(message.from_id, "❔ Какой сеанс хотите перенести?", 
    ...
    if orders == []:#  and closed_orders:
        ...
        for order in orders:
            with Session(engine) as session:
                schedule_event = session.scalars(select(ScheduleCalendar)
                    .where(ScheduleCalendar.id == order.schedule_id)).one()
                msg = f"🕸 Закак №{order.order_number}\n"\
...
def register_handlers_client_schedule(dp: Dispatcher):
    ...
    dp.register_message_handler(command_client_change_schedule_event, 
        Text(equals= kb_client.client_schedule_menu['change_event'], ignore_case=True), state=None)
    dp.register_message_handler(get_client_answer_to_change_schedule,
        state=FSM_Client_client_change_schedule_event.client_get_answer_yes_no)
    dp.register_message_handler(get_event_schedule_date,
        state=FSM_Client_client_change_schedule_event.get_order_number)
    dp.register_message_handler(get_new_event_schedule_date,
