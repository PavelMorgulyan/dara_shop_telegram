...
async def get_giftbox_item_command_list(message: types.Message): 
    if message.text.lower() == 'гифтбокс продукт' and \
        str(message.from_user.username) in ADMIN_NAMES:
        await message.reply('Какую команду гифтбокс продукта хочешь выполнить?',
...
async def command_load_giftbox_item(message: types.Message):
    if message.text.lower() in [ '/добавить_новый_гифтбокс', 'добавить новый гифтбокс'] \
        and str(message.from_user.username) in ADMIN_NAMES:
        await FSM_Admin_giftbox_item.giftbox_name.set()
        await message.reply('Введи название гифтбокса', reply_markup= kb_client.kb_cancel) 
...
async def load_giftbox_name(message: types.Message, state: FSMContext):
    ...
    await message.reply('А теперь загрузи фотографию гифтбокса', reply_markup= kb_client.kb_cancel) 
...
async def load_giftbox_photo(message: types.Message, state: FSMContext):
    ...
    await FSM_Admin_giftbox_item.next()
    await message.reply('Введи примерную цену на гифтбокс', reply_markup= kb_admin.kb_price)
...
async def load_giftbox_price(message: types.Message, state: FSMContext):
    if message.text in kb_admin.price:
        ...
        await FSM_Admin_giftbox_item.next()
        await message.reply('Введи описание гифтбокса', reply_markup= kb_client.kb_cancel) 
    else:
        await message.reply('Введи пожалуйста цену гифтбокса корректно - цифрами, слитно')
...
async def load_giftbox_note(message: types.Message, state: FSMContext):
    ...
    kb_candle_choice.add(KeyboardButton('Новую')).add(KeyboardButton('Выбрать из готовых'))
    await message.reply('Хорошо, теперь необходимо добавить свечу в гифтбокс.'\
...
async def get_giftbox_candle_name(message: types.Message, state: FSMContext):
    ...
    candle_item = []
    try:
        candle_item = await get_info_many_from_table('candle_items', 'name', candle_name)
        candle_in_table = True
    except:
        print('Свеча не в таблице')
        
    async with state.proxy() as data:
        data['giftbox_candle_name'], data['giftbox_candle_photo'], \
        data['giftbox_candle_price'], data['giftbox_candle_note'] = list(candle_item[0]) 
    ...
    if not candle_in_table:
        ...
        for i in range(4):
            await FSM_Admin_giftbox_item.next()
        await message.reply('Есть ли эти свечи сейчас в наличии или надо докупать?', 
...
async def giftbox_candle_state(message: types.Message, state: FSMContext): 
    ...
    await FSM_Admin_giftbox_item.next()
    await message.reply(f'Хорошо, а теперь добавь тему тату в этом гифтбоксе.'\
        f' На данный момент у тебя есть эти темы: {", ".join(tattoo_themes)}.'\
        f' Какую выбираешь?', reply_markup= kb_tattoo_theme)
...
async def load_giftbox_tattoo_note(message: types.Message, state: FSMContext):
    ...
    await FSM_Admin_giftbox_item.next()
    await message.reply('Есть ли эти тату сейчас в наличии или надо докупать?', 
        reply_markup=kb_admin.kb_in_stock) # in_stock_button = ['Есть в наличии', 'Нет в наличии, нужно докупать']
...
async def load_giftbox_sequins_type(message: types.Message, state: FSMContext):            
    ...
    await FSM_Admin_giftbox_item.next()
    await message.reply('Есть ли эти блестки сейчас в наличии или надо докупать?',
        reply_markup=kb_admin.kb_in_stock) # ['Есть в наличии', 'Нет в наличии, нужно докупать']
...
async def load_giftbox_sequins_state(message: types.Message, state: FSMContext):            
    ...
    await message.reply('Готово! Вы добавили гифтбокс в таблицу', reply_markup=kb_admin.kb_main)
...
async def command_get_info_giftboxes_item(message: types.Message): 
    if message.text.lower() in [ '/посмотреть_все_гифтбоксы', 'посмотреть все гифтбоксы'] \
        and str(message.from_user.username) in ADMIN_NAMES:
...
async def command_get_info_giftbox_item(message: types.Message):
    if message.text.lower() in [ '/посмотреть_гифтбокс', 'посмотреть гифтбокс'] \
        and str(message.from_user.username) in ADMIN_NAMES:
        orders_into_table = await get_info_many_from_table('giftbox_items')
...
async def get_name_for_info_giftbox_item(message: types.Message, state: FSMContext): 
    order_into_table = await get_info_many_from_table('giftbox_items', 'name', message.text)
...
async def command_change_price_giftbox_item(message: types.Message): 
    if message.text.lower() in [ '/поменять_цену_гифтбокса', 'поменять цену гифтбокса'] \
        and str(message.from_user.username) in ADMIN_NAMES:
        orders_into_table = await get_info_many_from_table('giftbox_items')
...
async def get_new_price_giftbox_item(message: types.Message, state: FSMContext): 
    ...
    await message.reply('На какую цену хочешь поменять?', reply_markup = kb_admin.kb_price)
...
async def set_new_price_giftbox_item(message: types.Message, state: FSMContext):
    if message.text.lower() != 'другая':
        ...
        await message.reply(f'Готово! Вы обновили цену  {giftbox_item_name} на {message.text}',
            reply_markup= kb_admin.kb_main)
        await state.finish() #  полностью очищает данные
    else:
        await FSM_Admin_change_price_giftbox_items.next()
...
#-------------------------------------------------------CREATE GIFTBOX ITEM------------------------------------------------------
    dp.register_message_handler(get_giftbox_item_command_list, commands='гифтбокс_продукт', state=None)
    dp.register_message_handler(get_giftbox_item_command_list,
        Text(equals='гифтбокс продукт', ignore_case=True), state=None)

    dp.register_message_handler(command_load_giftbox_item, commands='добавить_новый_гифтбокс', state=None)
    dp.register_message_handler(command_load_giftbox_item,
        Text(equals='добавить новый гифтбокс', ignore_case=True), state=None)
    dp.register_message_handler(load_giftbox_name, 
        state=FSM_Admin_giftbox_item.giftbox_name)
    dp.register_message_handler(load_giftbox_photo, content_types=['photo'], 
        state=FSM_Admin_giftbox_item.giftbox_photo)
    dp.register_message_handler(load_giftbox_price,
        state=FSM_Admin_giftbox_item.giftbox_price)
    dp.register_message_handler(load_giftbox_note, 
        state=FSM_Admin_giftbox_item.giftbox_note)
    dp.register_message_handler(get_giftbox_candle_choice,
        state=FSM_Admin_giftbox_item.giftbox_candle_choice)
    dp.register_message_handler(get_giftbox_candle_name, 
        state=FSM_Admin_giftbox_item.giftbox_candle_name)
    dp.register_message_handler(load_giftbox_candle_photo, content_types=['photo'], 
        state=FSM_Admin_giftbox_item.giftbox_candle_photo)
    dp.register_message_handler(load_giftbox_candle_price, 
        state=FSM_Admin_giftbox_item.giftbox_candle_price)
    dp.register_message_handler(giftbox_candle_note, 
        state=FSM_Admin_giftbox_item.giftbox_candle_note)
    dp.register_message_handler(giftbox_candle_state, 
        state=FSM_Admin_giftbox_item.giftbox_candle_state)
    dp.register_message_handler(load_tattoo_theme, 
        state=FSM_Admin_giftbox_item.giftbox_tattoo_theme)
    dp.register_message_handler(load_giftbox_tattoo_other_theme, 
        state=FSM_Admin_giftbox_item.giftbox_tattoo_other_theme)
    dp.register_message_handler(load_giftbox_tattoo_note, 
        state=FSM_Admin_giftbox_item.giftbox_tattoo_note)
    dp.register_message_handler(load_giftbox_tattoo_state, 
        state=FSM_Admin_giftbox_item.giftbox_tattoo_state)
    dp.register_message_handler(load_giftbox_sequins_type, 
        state=FSM_Admin_giftbox_item.giftbox_sequins_type)
    dp.register_message_handler(load_giftbox_sequins_state, 
        state=FSM_Admin_giftbox_item.giftbox_sequins_state)
    #-------------------------------------------------------COMMANDS GIFTBOX ITEM------------------------------------------------------

    dp.register_message_handler(command_change_price_giftbox_item, 
        commands='поменять_цену_гифтбокса', state=None)
    dp.register_message_handler(command_change_price_giftbox_item,
        Text(equals='поменять цену гифтбокса', ignore_case=True), state=None)
    dp.register_message_handler(get_new_price_giftbox_item, 
        state=FSM_Admin_change_price_giftbox_items.giftbox_item_number)
    dp.register_message_handler(set_new_price_giftbox_item, 
        state=FSM_Admin_change_price_giftbox_items.giftbox_item_price)
    dp.register_message_handler(set_new_other_price_giftbox_item, 
        state=FSM_Admin_change_price_giftbox_items.giftbox_item_other_price)
    
    dp.register_message_handler(command_get_info_giftboxes_item, 
        commands='посмотреть_все_гифтбоксы', state=None)
    dp.register_message_handler(command_get_info_giftboxes_item,
        Text(equals='посмотреть все гифтбоксы', ignore_case=True), state=None)
    
    dp.register_message_handler(command_get_info_giftbox_item, 
        commands='посмотреть_гифтбокс', state=None)
    dp.register_message_handler(command_get_info_giftbox_item,
        Text(equals='посмотреть гифтбокс', ignore_case=True), state=None)
    dp.register_message_handler(get_name_for_info_giftbox_item,
