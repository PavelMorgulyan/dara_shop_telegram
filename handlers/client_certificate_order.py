...
from handlers.client import CODE_LENTH, ORDER_CODE_LENTH, fill_client_table,\
    CALENDAR_ID, DARA_ID, FSM_Client_username_info
...
async def command_load_сert_item(message: types.Message):
    if message.text in ['Xочу сертификат 🎫', '/get_certificate']:
        ...
        await bot.send_message(message.from_id, '❔ На какую цену хотите сертификат?',
...
async def process_successful_cert_payment(message: types.Message, state=FSMContext):
    ...
    if message.content_type == 'text':
        ...
        if message.content_type == 'document':
            ...
            check_doc = await check_pdf_document_payment(user_id, price, doc_name, message.document.file_id)
            
        else:
            doc_name = doc_name[:-4] + '.jpg'
            file = message.photo[0].file_id
            
            check_doc = await check_photo_payment(message, user_id, price, doc_name, file) 
...
def register_handlers_client_cert(dp: Dispatcher):
    ...
    dp.register_message_handler(command_load_сert_item, commands=['get_certificate'], state=None)
    dp.register_message_handler(load_сert_price,
        state=FSM_Client_сert_item.сert_price)
    dp.register_message_handler(load_сert_payment_choice,
        state=FSM_Client_сert_item.сert_payment_choise)
    dp.register_message_handler(process_successful_cert_payment,
        content_types=['photo', 'document', 'message', 'text'],
        state= FSM_Client_сert_item.cert_payment_state_first)
    dp.register_message_handler(get_clients_cert_order,
        Text(equals=kb_client.choice_order_view["client_watch_cert_order"], ignore_case=True))
