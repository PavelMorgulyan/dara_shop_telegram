...
LIST_CANCEL_COMMANDS = ['отмена', '/отмена', '/cancel', 'cancel', 'отмена ❌', 'Отмена ❌'] 
...
MSG_NO_CORRECT_INFO_TO_SEND = 'Прости, но ты ввел неверные данные. '\
    'Пожалуйста, попробуй еще раз.'

MSG_START_TATTOO_ORDER = '🕸 Отлично, давай сделаем тебе тату! \n\n'

MSG_CLIENT_WANT_CURRENT_OR_NOT_TATTOO = '❔ Скажи, ты хочешь постоянное тату или переводное?'

MSG_CLIENT_BACK_AND_WHICH_PHOTO_DO_CLIENT_WANT_TO_LOAD = \
    '🔄 Вы вернулись назад в меню выбора загрузки тату эскиза\n\n'\
    '❔ Как хочешь загрузить фотографию эскиза для своего тату?'
    
MSG_CLIENT_LOAD_PHOTO = '📎 Хорошо, загрузи свой эскиз, пожалуйста. \n\n'\
    '❕ Можно добавлять сразу несколько файлов.'
    
MSG_CLIENT_SUCCESS_CHOICE_PHOTO = '📷 Отлично, ты выбрал(а) фотографию '\
    'эскиза для своего тату! \n\n'
    
MSG_WHICH_COLOR_WILL_BE_TATTOO = '❔ Это тату будет цветным или ч/б?'

MSG_CLIENT_DONT_HAVE_SKETCH_ORDERS = '⭕️ У тебя пока нет заказов эскизов для оплаты.\n\n'\
    '❔ Не хочешь ли оформить какой-нибудь заказ?'
    
MSG_CLIENT_DONT_HAVE_GIFTBOX_ORDERS = '⭕️ У тебя пока нет гифтбоксов для оплаты.\n\n'\
    '❔ Не хочешь ли оформить какой-нибудь заказ?'

MSG_GET_PHOTO_FROM_USER = \
    '🌿 Если у тебя нет идей для эскиза, ты можешь выбрать из готовых тату эскизов, '\
        'которые представлены в моей галерее. '\
        'Для этого нужно нажать кнопку \"Посмотреть галерею тату\". \n\n'\
    '🌿 Если хочешь свой эскиза для тату, то нажми кнопку \"Загрузить свою фотографию эскиза".\n\n'\
        '🌿 Или ты можешь нажать \"У меня нет фотографии для эскиза\", '\
        'чтобы оставить выбор фотографии тату на потом.\n\n'\
    '🌿 Чтобы вернуться в главное меню, необходимо нажать кнопку \"Отмена\"'

MSG_NO_CORRECT_INFO_LETS_CHOICE_FROM_LIST = '❌ Пожалуйста, выберете вариант из списка.'

CHECK_LIST = [ 'Чек по операции', 'Перевод с карты на карту', 'Платёж', 'Платеж', 'Перевод']

MSG_CANCEL_ACTION = '❌ Вы отменили действие. \n\n'

GIFTBOX_DESCRIPTION = ' Отлично, давайте выберем гифтбокс.\n' \
...
MSG_TO_CHOICE_CLIENT_PHONE = 'Осталось только добавить свой телефон. \n\n '\
    '☎️ Нажми кнопку \"Отправить свой контакт ☎️\", '\
    'чтобы заказ никуда не потерялся, и у меня всегда была связь с тобой)\n\n'\
    '⭕️ Или ты можешь не оставлять свой телефон. '\
    'Для этого нажми кнопку \"Не хочу оставлять свой телефон, только телеграм 💬\"'

MSG_NO_PRICE_LIST_IN_DB = '❌ Прости, у тебя пока нет прайс-листа.\n'\
    'Его необходимо заполнить c помощью функции '\
    '\"создать новый прайс-лист на тату\". '

MSG_WHITCH_ORDER_WANT_TO_SEE_CLIENT = '❔ Какие заказы хочешь посмотреть?'

MSG_BACK = '🔄 Хорошо, вы вернулись назад. '

MSG_ABOUT_TATTOO_MASTER = 'Привет! ‌Меня зовут Дара, и я верю, что наша встреча не случайна)\n\n'\
...
MSG_SCRETH_DEV = 'Индивидуальный эскиз готовится заранее. '\
    ...
    'помощью переводной татуировки — напишите мне, стоимость составит от 1000р до 3500р'
    
MSG_CONTRAINDICATIONS = 'Постоянные\n'\
...
MSG_PREPARING_TATTOO_ORDER = 'Подготовка к сеансу состоит из следующих действий:\n'\
...
MSG_COUPLE_TATTOO = 'Эскиз для парной/тройной/четверной/и т.д.'\
    ' тату согласовывается со всеми участниками процесса.\n\n'\
    'Для этого создаётся отдельный чат в удобном мессенджере.'
    
MSG_TATTOO_CARE = 'В конце сеанса я заклею вашу татуировку заживляющей плёнкой, '\
...
MSG_COOPERATION = 'Я открыта для сотрудничества, '\
...
MSG_NO_SCHEDULE_CONSULTATION = \
    '❌ Прости, но на этот месяц пока нет свободных дат'\
    ' для консультаций.\n\n '\
    '🗓 А пока, при заказе консультации татуировки ты можешь выбрать '\
    'свою дату, которая удобна для тебя.\n\n'\
    '💬 Не переживай, позже я с тобой свяжусь!'
    
MSG_WHICH_INFO_DO_CLIENT_WANT_TO_GET = '❔ Какую ещё информацию хочешь получить?'

MSG_DO_CLIENT_WANT_TO_DO_MORE = '❔ Хочешь сделать что-нибудь еще?'

MSG_THANK_FOR_ORDER = '🎉 Спасибо, что сделали заказ!\n💬 '\
    'Я обязательно свяжусь с вами в ближайшее время!'

MSG_TATTOO_ORDER_CLIENT_CHOICE_BODY_LATER = 'Не проблема, выберем с тобой место для тату позже.'

MSG_CLIENT_WANT_TO_FILL_ORDER_NOTE = '❔ Хочешь чего-нибудь добавить к описанию своего заказа?'

CLIENT_WANT_TO_CHANGE_MORE = '❔ Хочешь что-нибудь еще изменить в этом тату?\n\n' \
    'Если ничего менять не хочешь, необходимо нажать кнопку \"Ничего не хочу менять ➡️\" для продолжения'

MSG_NO_ORDER_IN_TABLE = '⭕️ Пока у вас нет заказов в таблице.'

MSG_NO_CERT_TO_PAY = '⭕️ У тебя пока нет сертификатов для оплаты.\n\n'\
    '❔ Не хочешь ли оформить какой-нибудь заказ?'

MSG_WHICH_ORDER_DO_CLIENT_WANT_TO_PAY = '❔ Какие заказы хочешь оплатить?'

MSG_WHICH_ORDER_DO_CLIENT_WANT_TO_SEE = "❔ Какой заказ хочешь посмотреть?"

MSG_NO_TATTOO_IN_TABLE = '⭕️ Прости, но у тебя еще нет тату в таблице.'

MSG_NO_SCHEDULE_IN_TABLE = '⭕️ Прости, но у тебя еще нет дат в расписании в таблице в этом статусе.'

MSG_NO_SCHEDULE_PHOTO_IN_TABLE = '⭕️ Прости, но у тебя еще нет фотографий расписания в таблице.'

MSG_TO_GET_SCHEDULE = '🗓 Мое расписание появилось выше.\n❕ Выбери дату из расписания, пожалуйста. '

MSG_TO_NO_SCHEDULE = \
    'Пока дат для тату в этом месяце нет, но мы можем обсудить дату для тату в следующем месяце уже лично. '\
    'Нажми кнопку \"Далее\"'
...
MSG_CLIENT_HAVE_NEW_DATE_IN_TATTOO_ORDER = \
    ...
    'Дата поменялась с %s на %s, а время начала с %s на %s, а время окончания с %s на %s! Жду вас на сеансе!\n\n'\
    'По всем вопросам обращайтесь к https://t.me/dara_redwan'
...
MSG_CHANGE_SCHEDULE_STATUS_ACTIONS_INFO = \
    ...
    '1) Поставить неизвестную дату в заказе (обнулить дату). '\
        ...
        'В этом случае, если админ выбирает оповещение, то Клиенту приходит уведомление следующего характера:\n'\
        '\"Уважаемый клиент <имя>! Ваше посещение тату салона по заказу <номер заказа> теперь имеет неопределенную дату. '\
    ...
    'В 2 и 3 вариантах, если админ выбирает оповещение, к клиенту приходит следующее сообщение:'\
        '\"Уважаемый клиент <имя>! Ваше посещение тату салона по заказу <номер заказа> теперь имеет другую дату и время!'\
...
MSG_DO_ADMIN_WANT_TO_NOTIFY_CLIENT = 'Хочешь оповестить пользователя об изменении в расписании?'

MSG_PLS_SEND_TATTOO_PHOTO_OR_CANCEL_ACTION = \
    '📎 Пожалуйста, загрузи фотографию тату через файлы или отмени действие'

MSG_CLIENT_DO_WANT_ADD_ANOTHER_PHOTO_OR_VIDEO_BODY = '❔ Хочешь добавить еще фотографию или видео?'

MSG_CLIENT_GO_BACK = '🔄 Вы вернулись назад\n\n'

MSG_MY_FREE_CALENDAR_DATES = 'Вот мои свободные даты в этом месяце:\n'

MSG_CLIENT_CHOICE_TATTOO_SIZE = '📏 Выбери примерный размер тату, пожалуйста.'

MSG_CLIENT_CHOICE_TATTOO_NAME = '❔ Напиши, какое имя было бы у твоего тату?\n\n'\
    '❕ Имя тату должно быть коротким - не более 2-3 слов.'

MSG_GET_DESCRIPTION_TATTOO_FROM_CLIENT = '💬 Хорошо, давай попробуем создать эскиз тату из твоего описания.\n\n'

MSG_GET_DESCRIPTION_TATTOO_FROM_ADMIN = 'Хорошо, давай попробуем создать изображение модели девушки из твоего описания.\n\n'
...
MSG_WHICH_TATTOO_WANT_TO_CHOOSE = '❔ Какое тату хочешь выбрать?'

MSG_GET_DESCRIPTION_ABOUT_WORDS = "В данной функции при генерации изображения из текста доступно '\
    'множество описательных слов"

MSG_START_INSTRUCTION_OF_GENERATING_AI_IMGS = 'Для того, чтобы правильно сгенерировать изображение, '\
...
MSG_GET_DESCRIPTION_WOMAN_MODEL_FROM_ADMIN_CONCEPTS = \
    ...
    ' которые хочешь увидеть на модели: часы, чокер, виснушки. Пример: woman with frills and a watch on her hand.\n\n'\
    ...
    'Подробнее о них ты можешь ознакомиться здесь: https://promptomania.com/stable-diffusion-prompt-builder/\n\n'\
...
MSG_STYLES_CHROMATIC_PALETTES = \
    'Например, вот список цветового окружения, которое ты можешь добавить в свой текст:\n\n'\
    'chromatic palettes:\n - Warm Color palette\n - Cool\n - Colofur\n'\
    '- Rainbow\n- Spectral\n- Inverted\n- Chroma\n- Dichromatism\n- Tetrachromacy\n- Saturated\n- Neon'\
    'Electric\n- Tonal\n- Complimentary\n- Split-Complimentary-\n- Supplementary\n- Analogous\n- Triadic'\
    'Tetradic\n- Polychromatic\n- Light\n- Dark\n- Light Mode\n- Dark Mode\n- Tones of Black'\
...
MSG_MOTION_PICTURE_PROCESS = \
'motion picture process:\n- Technicolo\n- Kinemacolor\n- Kodachrome\n- Cinecolor\n- Agfacolor'

MSG_ADD_SYMBOLS_FOR_GETTING_STRONGER_WORD = '!!! Ставь слова в скобки, если хочешь усилить или '\
...
MSG_EXAMPLE_TEXT_PORTRAIT = 'beautiful cute front portrait of an elvish deity, long hair in motion, herbs and blue pastel colors, '\
...
MSG_EXAMPLE_TEXT_PORTRAIT_WOMAN_MODEL_IN_HOME = \
    ...
    '<1:1.5><2000:1.0> --scale 11.0 ' # --steps 50 --H 512 --W 512 --scale 11.0

MSG_EXAMPLE_TEXT_PORTRAIT_WOMAN_MODEL_IN_HOME_EASY = 'full body woman with freckles in casual modern boho '\
    'free clothes in modern home in boho style'
    
MSG_EXAMPLE_TEXT_BACK_WOMAN_MODEL_IN_HOME_EASY = 'woman\'s back in casual modern boho free clothes in '\
    'modern home in boho style'

WOMAN_BOHO_STYLE_DESC = 'a long-range realistic photo of %s (open shoulders:1.6), (open belly:1.1), standing sideways, short haircut,'\
...
MSG_ANSWER_AOUT_RESULT_TATTOO_FROM_AI = 'Вот твоя фотография. Хочешь попробовать еще? '\
    'Или данное изображение тебя устраивает?'

MSG_NOT_CORRECT_DATE_NOW_LESS_CHOICEN = 'Прости, но выбранная дата стоит раньше нынешнего дня.\n\n'

MSG_LET_CHOICE_NORMAL_DATE = 'Выбери подходящую дату'

MSG_FILL_TEXT_OR_CHOICE_VARIANT = 'Введи текст для изображения или выбери вариант, пожалуйста.'

MSG_ERROR_CHECK_VALIDATE_ADMIN_NAME = "Кажется, вы отправили не тому человеку, '\
    'т.к. имя получателя не совпадает.\n"\
    "Пожалуйста, отправь лично мне на указанный телефон. '\
    'Там должно стоять имя \"Дария Редван Э\""

MSG_ERROR_CHECK_VALIDATE_ORDER_PRICE = "Кажется, вы отправили чек не на ту сумму, '\
    'т.к. сумма в этом чеке не совпадает.\n Пожалуйста, отправь чек на сумму %s"
    
MSG_ERROR_CHECK_VALIDATE_NOT_CORRECT_ADMIN_PHONE_NUMBER = "Кажется, вы отправили чек не на тот мобильный номер, '\
    'т.к. номер в этом чеке не совпадает с моим.\n"\
    f"Пожалуйста, отправь чек на сумму %s на номер телефона +7(925)885-07-87"
...
MSG_ERROR_CHECK_VALIDATE_NOT_CORRECT_CHECK_DOCUMENT = \
    ...
    f"Пожалуйста, отправь именно чек о переводе на сумму %s на номер телефона +7(925)885-07-87"    